...
file_meta <- "/data/data2/Projects/Fusion/fusiondb_v4/metafusion_gdc_divide.tsv"
metafusion <- fread(file_meta , sep = "\t")
cohort1 <- fread("/data/data2/Projects/Fusion/fusiondb_v4/cohorts_meta.csv")
# Stratification Strategy
## 1. For tumor samples, if there are at least two samples within the total cohorts of a disease that have detected the same fusion value, +1 score.
## 2. If one fusion event's max_split_cnt and max_span_cnt are both greater than or equal to 1, +1 score;
## 3. If Arriba and STAR-Fusion detected identical fusion event, +1 score; 
## 4. score >= 2 , high confident fusion, else low confident fusion
Tumor_data <-  metafusion  |>
  filter(!(gene1 %in% c("IGH@-ext", "IGL@-ext", "IGL-@-ext", "IGH-@-ext") | 
           gene2 %in% c("IGH@-ext", "IGL@-ext", "IGL-@-ext", "IGH-@-ext"))) |> 
               mutate(
                Score = 0,
                fusion = paste0(gene1,"::",gene2),
                disease = case_when(
                  grepl( "TCGA", cohort) ~ gsub("^TCGA-", "", cohort),
                  grepl( "TARGET", cohort) ~ gsub("^TARGET-", "", cohort),
                  grepl( "CPTAC", cohort) ~ gsub("^CPTAC-", "", cohort),
                  !cohort %in% c("TCGA", "TARGET", "CPTAC") ~ cohort1$cancer_type[match(cohort, cohort1$id)],
                  TRUE ~ NA
                ),
                disease = case_when(
                  disease %in% c("LUSC", "LUAD") ~ "NSCLC",
                  TRUE ~ disease
                )
              ) 
...
Tumor_data1 <- Tumor_data %>%
  group_by(disease, fusion) %>%
  mutate(Score = case_when(  
    sample_type %in% c("Tumor", "Blood") & n_distinct(Sample_ID) >= 2 ~ Score + 1,
    TRUE ~ Score)
  ) %>%
  ungroup() 
...
Tumor_data2 <- Tumor_data1 %>%
  mutate(
    Score = case_when(
    max_split_cnt >= 1 & max_span_cnt >= 1 ~ Score + 1,
      TRUE ~ Score
    )
  )
...
Tumor_data3 <- Tumor_data2 %>%
    mutate(
      Score = case_when(
           tools == "arriba,starfusion" ~ Score + 1,
           TRUE ~ Score)
          )

# Annotate Strategy
## Annotate the gene types of the 5' gene and 3' gene based on the /gencode.v34.annotation.gtf file
gtf = fread("/data/data2/reference/hg38_ek12/gencode.v34.annotation.gtf")
...
gtf[, gene_type := extract_col(V9, "gene_type")]
table(gtf$gene_type)
...
df <- gtf_gene %>%
  mutate(
    gene_id = str_replace(str_extract(V9, 'gene_id ".*?";'), 'gene_id "|";', ''),
    gene_name = str_replace(str_extract(V9, 'gene_name ".*?";'), 'gene_name "|";', '')
  ) |>
mutate(
...
## Annotate the gene types of the 5' gene and 3' gene based on the ucsc genome browser and genecards
match_type <- c(
  "AL357556.3" = "lncRNA",
  "Y_RNA" = "misc_RNA",
  "PDCD6-AHRR"= "ncRNA",
  ...
  "ZRANB2-DT"= "lncRNA", 
  "MIR4432HG"= "lncRNA", 
  "CTD-2194D22.4"= "lncRNA", 
  "SMASR"= "lncRNA", 
  "CFAP20DC-AS1"= "lncRNA",
  "TMEM139-AS1"= "lncRNA", 
  "FLJ12825"= "lncRNA", 
  "RIPOR3-AS1"= "lncRNA",
  "LL22NC03-63E9.3"= "lncRNA", 
  "HRAT17"= "lncRNA", 
  "LINC01377"= "lncRNA", 
  "FLJ46284"= "lncRNA", 
  "SMC2-DT"= "lncRNA", 
  ...
  "FLJ42969"   = "lncRNA",
  "FLJ31356"   = "ncRNA",
  "Metazoa_SRP" = "misc_RNA",
  "AC000050.2" = "lncRNA", 
  "AC007461.1"  = "lncRNA",
  "AC020765.6" = "protein_coding", 
  "AC026748.7"  = "lncRNA",
  "AC069544.2" = "protein_coding",
  "AC105001.2"  = "protein_coding",
  "AC114744.2" = "lncRNA",
  "AC137894.4" = "lncRNA",
  "AC183088.4" = "lncRNA",
  "AC233280.36" = "unprocessed_pseudogene",
  "AL136295.22" = "lncRNA",
  "AP000356.5" = "protein_coding",
  "AL391628.1" = "protein_coding",
  "AC006486.3"  = "protein_coding",
  "AC093246.1"  = "protein_coding",
  "AL713999.2" = "protein_coding",
  "AC018630.4"  = "protein_coding",
  "AC133555.6"  = "lncRNA",
  "AC106873.8"  = "protein_coding",
  "AC084756.2"  = "protein_coding",
  "AC008763.4"  = "protein_coding",
  "AC134684.11" = "unprocessed_pseudogene",
  "AC067945.4" = "lncRNA",
  "AP000721.2"  = "lncRNA",
  "AL096814.2"  = "protein_coding",
  "AC004510.2"  = "lncRNA",
  "AL096870.12" = "lncRNA",
  "AC011455.8"  = "lncRNA",
  "AC084018.3"  = "lncRNA",
  ...
  "AC113189.12" = "lncRNA",
  "AC009093.11"= "lncRNA",
  "AC092032.2" = "transcribed_unprocessed_pseudogene",
  "AC113143.3"  = "lncRNA",
  "AC020929.1"  = "lncRNA",
  "FAM86C1P" = "pseudogene",
"BMAL1" = "protein_coding",
"G6PC1" = "protein_coding",
"GBA1" = "protein_coding",
"GBA1LP" = "pseudogene",
"GGTA1" = "protein_coding",
"CMKLR2" = "protein_coding",
"IRAG2" = "protein_coding",
"MTX1LP" = "pseudogene",
"PHB1" = "protein_coding",
"SNORD3P1" = "pseudogene",
"SKIC2" = "protein_coding",
"TAFAZZIN" = "protein_coding",
"DYNLT2" = "protein_coding",
"GFUS" = "protein_coding",
"ZNF56P" = "pseudogene",
"PRP4K" = "protein_coding",
"NHERF2" = "protein_coding",
"NHERF1" = "protein_coding",
"ENTREP1" = "protein_coding",
"ATP5MJ" = "protein_coding",
"SKIC3" = "protein_coding",
"BLTP2" = "protein_coding",
"GARRE1" = "protein_coding",
"IRAG1" = "protein_coding",
"ENTREP3" = "protein_coding",
"POLR1G" = "protein_coding",
"GUSBP15" = "pseudogene",
"CEP43" = "protein_coding",
"BLTP3B" = "protein_coding",
"SSPOP" = "pseudogene",
"KATNIP" = "protein_coding",
"ENTREP2" = "protein_coding",
"MATCAP2" = "protein_coding",
"RIGI" = "protein_coding",
"DNAAF11" = "protein_coding",
"TMT1A" = "protein_coding",
"DNAAF9" = "protein_coding",
"ZZZ3" = "protein_coding",
"FBXL9P" = "pseudogene",
"HAPSTR1" = "protein_coding",
"CFAP263" = "protein_coding",
"MYL11" = "protein_coding",
"ZFTRAF1" = "protein_coding",
"METTL25B" = "protein_coding",
"DUSP13B" = "protein_coding",
"METTL13" = "protein_coding",
"IFT25" = "protein_coding",
"SMIM11" = "protein_coding",
"BLTP3A" = "protein_coding",
"BPNT2" = "protein_coding",
"ADISSP" = "protein_coding",
"MTNAP1" = "protein_coding",
"NTAQ1" = "protein_coding",
"DYNC2I1" = "protein_coding",
"ODAD2" = "protein_coding",
"DNAI7" = "protein_coding",
"VNN3P" = "pseudogene",
"FIRRM" = "protein_coding",
"ZNG1A" = "protein_coding",
"ATOSA" = "protein_coding",
"SLC19A4P" = "pseudogene",
"BMAL2" = "protein_coding",
"FERRY3" = "protein_coding",
"LDAF1" = "protein_coding",
"GFOD3P" = "pseudogene",
"CEP15" = "protein_coding",
"CFAP92" = "protein_coding",
"ELAPOR1" = "protein_coding",
"NHSL3" = "protein_coding",
"FHIP2A" = "protein_coding",
"MYG1" = "protein_coding",
"FAM200C" = "protein_coding",
"STEEP1" = "protein_coding",
"PALS1" = "protein_coding",
"RUSF1" = "protein_coding",
"FHIP2B" = "protein_coding",
"CFAP68" = "protein_coding",
"SLC26A10P" = "pseudogene",
"CENPBD2P" = "pseudogene",
"ZFTA" = "protein_coding",
"AFG2B" = "protein_coding",
"INTS15" = "protein_coding",
"BBLN" = "protein_coding",
"CSTPP1" = "protein_coding",
"BRME1" = "protein_coding",
"CLXN" = "protein_coding",
"TOP6BL" = "protein_coding",
"SPRING1" = "protein_coding",
"DNAI4" = "protein_coding",
"IFT56" = "protein_coding",
"TASL" = "protein_coding",
"ATOSB" = "protein_coding",
"PHAF1" = "protein_coding",
"SKIC8" = "protein_coding",
"POLR1HASP" = "pseudogene",
"YJU2B" = "protein_coding",
"ODAD4" = "protein_coding",
"ARB2A" = "protein_coding",
"FHIP1B" = "protein_coding",
"BLTP1" = "protein_coding",
"SPATA31H1" = "protein_coding",
"QNG1" = "protein_coding",
"MFSD14CP" = "pseudogene",
"CDIN1" = "protein_coding",
"SANBR" = "protein_coding",
"HYCC1" = "protein_coding",
"SPMIP6" = "protein_coding",
"GARIN1B" = "protein_coding",
"C19orf48P" = "pseudogene",
"VCF1" = "protein_coding",
"SLX9" = "protein_coding",
"CFAP91" = "protein_coding",
"DYNC2I2" = "protein_coding",
"VCF2" = "protein_coding",
"RLIG1" = "protein_coding",
"MTRFR" = "protein_coding",
"STYXL2" = "protein_coding",
"KGD4" = "protein_coding",
"ODAD1" = "protein_coding",
"GOLM2" = "protein_coding",
"ZBED10P" = "pseudogene",
"DNAAF10" = "protein_coding",
"NOPCHAP1" = "protein_coding",
"ABTB3" = "protein_coding",
"CEP20" = "protein_coding",
"KRBOX5" = "protein_coding",
"CHCT1" = "protein_coding",
"TEKTL1" = "protein_coding",
"DNAI3" = "protein_coding",
"CFAP276" = "protein_coding",
"OR14L1" = "protein_coding",
"CIMAP3" = "protein_coding",
"CIMIP1" = "protein_coding",
"CFAP210" = "protein_coding",
"RPLP1P5" = "pseudogene",
"MIX23" = "protein_coding",
"CFAP90" = "protein_coding",
"SPMIP4" = "protein_coding",
"CIBAR1" = "protein_coding",
"CFAP95" = "protein_coding",
"SPATA31G1" = "protein_coding",
"DNAAF6" = "protein_coding",
"MTCL2" = "protein_coding",
"CFAP251" = "protein_coding",
"KICS2" = "protein_coding",
"DNAAF8" = "protein_coding",
"ARK2N" = "protein_coding",
"ZNF285BP" = "pseudogene",
"KASH5" = "protein_coding",
"NTMT2" = "protein_coding",
"ZDHHC8BP" = "pseudogene",
"ZNG1B" = "protein_coding",
"SLC66A1LP" = "pseudogene",
"SPMIP2" = "protein_coding",
"GARIN3" = "protein_coding",
"CFAP418" = "protein_coding",
"BRD10" = "protein_coding",
"PABIR2" = "protein_coding",
"PABIR3" = "protein_coding",
"TAMALIN" = "protein_coding",
"GARIN2" = "protein_coding",
"CIMAP2" = "protein_coding",
"AFG2A" = "protein_coding",
"GARIN6" = "protein_coding",
"DYNLT5" = "protein_coding",
"SPMIP3" = "protein_coding",
"ALG1L1P" = "pseudogene",
"CFAP20DC" = "protein_coding",
"PRSS59P" = "pseudogene",
"SAXO4" = "protein_coding",
"ZNG1E" = "protein_coding",
"POLR1F" = "protein_coding",
"ELAPOR2" = "protein_coding",
"LHFPL7" = "protein_coding",
"SPMIP11" = "protein_coding",
"DYNLT2B" = "protein_coding",
"SPATA31F1" = "protein_coding",
"DRC12" = "protein_coding",
"REDIC1" = "protein_coding",
"GREP1" = "protein_coding",
"PDXDC2P-NPIPB14P" = "pseudogene",
"CCDC144BP" = "pseudogene",
"PTGR3" = "protein_coding",
"ACTMAP" = "protein_coding",
"CIMAP1D" = "protein_coding",
"EEIG2" = "protein_coding",
"HYCC2" = "protein_coding",
"RIMOC1" = "protein_coding",
"CSGALNACT2P1" = "pseudogene",
"FAM3C2P" = "pseudogene",
"GCATP1" = "pseudogene",
"SSX13P" = "pseudogene",
"DUSP29" = "protein_coding",
"CENATAC" = "protein_coding",
"CIMIP4" = "protein_coding",
"IHO1" = "protein_coding",
"AKR1C8" = "protein_coding",
"POU2AF2" = "protein_coding",
"CRACDL" = "protein_coding",
"GARIN1A" = "protein_coding",
"CASTOR3P" = "pseudogene",
"PKD1P4" = "pseudogene",
"USP41P" = "pseudogene",
"FBXW10B" = "protein_coding",
"SPMIP8" = "protein_coding",
"SAXO5" = "protein_coding",
"MTCL3" = "protein_coding",
"PEDS1" = "protein_coding",
"PEDS1-UBE2V1" = "protein_coding",
"GPR15LG" = "protein_coding",
"SPMAP1" = "protein_coding",
"MEIOSIN" = "protein_coding",
"SMIM34" = "protein_coding",
"EEIG1" = "protein_coding",
"LIAT1" = "protein_coding",
"SPACDR" = "protein_coding",
"TEX56P" = "pseudogene",
"CARD17P" = "pseudogene",
"TTLL13" = "protein_coding",
"CIMAP1B" = "protein_coding",
"UQCC5" = "protein_coding",
"CFAP96" = "protein_coding",
"ARK2C" = "protein_coding",
"TPRX2" = "protein_coding",
"TUBAP13" = "pseudogene",
"HNRNPA1L3" = "protein_coding",
"FAM25EP" = "pseudogene",
"PAFAH1B2P2" = "pseudogene",
"CASP4LP" = "pseudogene",
"CCDC121P1" = "pseudogene",
"ZNG1F" = "protein_coding",
"UQCR10P1" = "pseudogene",
"CBX3P2" = "pseudogene",
"FAM90A12" = "protein_coding",
"RHPN2P1" = "pseudogene",
"OSTCP3" = "pseudogene",
"ZNF285CP" = "pseudogene",
"ZKSCAN8P1" = "pseudogene",
"MATCAP1" = "protein_coding",
"CSPG4BP" = "pseudogene",
"NALF1" = "protein_coding",
"UQCC6" = "protein_coding",
"FHIP1A" = "protein_coding",
"RPL35AP20" = "pseudogene",
"SRSF6P1" = "pseudogene",
"CIROP" = "protein_coding",
"SPATA31F3" = "protein_coding",
"JPT1P1" = "pseudogene",
"SPMIP7" = "protein_coding",
"ZNF705EP" = "pseudogene",
"GOLGA6L3P" = "pseudogene",
"GUSBP13" = "pseudogene",
"NME2P3" = "pseudogene",
"B3GALT9" = "protein_coding",
"FMN2P1" = "pseudogene",
"ALOXE3P1" = "pseudogene",
"CDRT15P6" = "pseudogene",
"KPNA4P1" = "pseudogene",
"RBX1P2" = "pseudogene",
"LASP1NB" = "protein_coding",
"PPP5D1P" = "pseudogene",
"MISFA" = "protein_coding",
"SPMAP2L" = "protein_coding",
"TBC1D3JP" = "pseudogene",
"DNAJB6P3" = "pseudogene",
"NOTCH2NLR" = "protein_coding",
"C3orf86P" = "pseudogene",
"PKD1P6-NPIPP1" = "pseudogene",
"NFILZ" = "protein_coding",
"MALLP2" = "pseudogene",
"CSGALNACT2P2" = "pseudogene",
"MTND6P35" = "pseudogene",
"BMS1P4-AGAP5" = "pseudogene",
"ABCF2-H2BK1" = "protein_coding",
"MS4A19P" = "pseudogene",
"TBCEL-TECTA" = "protein_coding",
"FAM246C" = "protein_coding"
)


match_df <- data.frame(
  gene_name = names(match_type),
  gene_type = match_type,
...
gene_type_map <- match_all %>%
  distinct(gene_name, gene_type) %>%
  deframe()

fusion_distribution <-  Tumor_data3 %>%
  filter(!(gene1 %in% c("IGH@-ext", "IGL@-ext", "IGL-@-ext", "IGH-@-ext") | 
           gene2 %in% c("IGH@-ext", "IGL@-ext", "IGL-@-ext", "IGH-@-ext"))) %>%
  mutate(
    gene1_type = sapply(str_split(gene1, ","), function(genes) {
      types <- gene_type_map[genes]
      paste(types[!is.na(types)], collapse = ",")
    }),
    gene2_type = sapply(str_split(gene2, ","), function(genes) {
      types <- gene_type_map[genes]
      paste(types[!is.na(types)], collapse = ",")
    }
    ) )
gene2_nc <- fusion_distribution |> filter(is.na(gene2_type))  |> select(gene2) |> unique()
gene1_nc <- fusion_distribution |> filter(is.na(gene1_type)) |> select(gene1) |> unique()
nc <- c(gene1_nc$gene1, gene2_nc$gene2)  |> unique()
nc 
# New metafusion
metafusion_new <- fusion_distribution  |>  
                  select(-c(disease,fusion)) 
              

file_name <- "/data/data2/Projects/Fusion/fusiondb_v4/metafusion_calls.star.tsv"
